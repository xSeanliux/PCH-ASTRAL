...
- [ ] **CLI arg inconsistency**: `experiment status`/`compact` take the experiment *folder*; `experiment inference`/`score` take the *spec yaml*. Trips users (passed yaml to `compact` → `NotADirectoryError`). Unify — accept either (spec → derive folder), or make all four take the same thing.
- [ ] **Thin `experiment status`**: only a per-method run tally — no per-dataset or expected-vs-done view. Richer "what's missing" inspector is the PR2 SLURM `status`; until then users hand-join CSVs to see gaps.
