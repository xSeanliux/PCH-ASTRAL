...
- [ ] **GA/MrBayes blocked**: smoke test showed MrBayes (`bin/mb`, 3.2.7a) rejects the generated nexus's `mcmcp … filename=Bayes_out_<runid>` with `Error when setting parameter "Filename" (2)`, so GA produces no `trees1/*.trees`. In the legacy `scripts/R/commandLineNex.R` / `runGA.sh` toolchain (not the CLI). Root-cause needs a live `mb` run on the generated `.nex`. (MP4 + ASTRAL3 verified runnable after the runGA/runASTRAL3/types fixes.)
