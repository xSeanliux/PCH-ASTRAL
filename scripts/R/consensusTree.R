...
option_list = list(
    ...
    make_option(c("-d", "--discard"), type='integer', default=0,
                help='integer betweeen 1 and 100, will discard floor(no. of trees * 100/discarded) of the first trees.', metavar='number')
...
discard = as.integer(opt[['discard']])
...
returnTreeSetForMultiple <- function(trees, method, discard) {
  if(discard != 0) {
    to_discard = floor(discard * length(trees) / 100)
    print(c("discarding ", to_discard, " trees out of ", length(trees)))
    trees = tail(trees, length(trees) - to_discard) # keep the last N - to_discard trees
  }
...
if (length(trees) == 1) print("Only one tree detected; returning that tree.")
if (length(trees) > 1) trees <- returnTreeSetForMultiple(trees, resolve_multiple, discard)

outputfile = opt[['output-file']]
if (!file.exists(outputfile)) {
  file.create(outputfile)
}

write.tree(trees, outputfile)