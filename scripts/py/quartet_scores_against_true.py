...
QUARTETSCORERS = [QuartetScorer(tree) for tree in TREEOBJS]
...
for qmode, ef, hf, cf, poly, tree, rep in product(
    ...
    data_path = (
        OMP
        / "example"
        / "simulated_data_theorypaper"
        / f"{poly}_{hf}_{ef}_{cf}"
    ...
    astral_log_path = (
        OMP
        / "sim_outputs_theorypaper"
        / f"{poly}_{hf}_{ef}_{cf}"
        / f"ASTRAL({qmode},5)"
        / "logs"
        / f"sim_tree{tree}_{rep}.log"
    )

    with open(astral_log_path, "r") as af:
        lines = af.readlines()
        fqsl = [line for line in lines if line.startswith("Final quartet score is:")]
        assert len(fqsl) == 1
    ...
    print(
        f"{score}\t{astral_quartet_score}\t{total_quartets} = {score / total_quartets:.4f} \t{astral_quartet_score / total_quartets:.4f}."
    )
...
df.to_csv(
    OMP / "sim_outputs_theorypaper" / "astral_stats" / "astral_stats_master.csv",
    index=False,
)
