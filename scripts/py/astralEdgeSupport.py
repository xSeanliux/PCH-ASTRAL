...
def test_astral(tree_path, quartets):
    ...
    result = subprocess.run(
        ...
        + ["-i", quartet_file, "-q", tree_path, "-t", "1"],
        capture_output=True,
        text=True,
    )
    ...
    total_quartets = int(
        re.search(
            r"Number of quartet trees in the gene trees: (?P<total_quartets>\d+)",
            result,
        ).group("total_quartets")
    )
    score = int(
        re.search(r"Final quartet score is: (?P<score>\d+)", result).group("score")
    )
    return score, total_quartets


def test_quartets(tree_path, quartets: List[Tuple[Tuple, int]]):
...
def main():
    ...
    try:
        ...
        _, quartets = get_quartets(csv_path=data_path)
        all_quartets = list(quartets.items())
        r = test_astral(tree_path=tree_path, quartets=all_quartets)
        print(r)
