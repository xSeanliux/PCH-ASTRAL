                    ...
                    for ((r=1;r<=$REPLICA_COUNT;r++)); do # rep number
                        ...
                        if $DO_GA; then 
                            if test -s $TREEOUTPUT/GA/trees1/$id.trees; then 
                                bash $TALLIS/OneMostProb/scripts/sh/runGA.sh\
                                    --runid $RUNID\
                                    --input $FILE\
                                    --output $TREEOUTPUT\
                                    --name $id
                                ...
                                Rscript $TALLIS/OneMostProb/scripts/R/RFScorer.R -f nexus -r $CURRENT_TREE -m 4 -p 0 -i $TREEOUTPUT/GA/trees1/$id.trees >> $GA_SCOREOUTPUT
                            else
                                echo "skipping scoring "$id
                            fi
                        fi
