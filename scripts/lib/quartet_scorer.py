...
class QuartetScorer:
    ...
    def test_quartet(self, q: Quple) -> bool:
        ...
        return not (
            clade_eq(l, l2)
            and (
                clade_eq(self.tree.common_ancestor([a, b, c]), l1)
                or clade_eq(self.tree.common_ancestor([a, b, d]), l1)
            )
            or clade_eq(l, l1)
            and (
                clade_eq(self.tree.common_ancestor([c, d, a]), l2)
                or clade_eq(self.tree.common_ancestor([c, d, b]), l2)
            )
        )
