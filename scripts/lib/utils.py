...
def get_weights(csv_name):
    ...
    if not all(
        isinstance(x, int) or (isinstance(x, float) and x.is_integer()) for x in weights
    ):
        raise ValueError(
            "All values in 'weight' must be integers or convertible to integers without loss."
        )
...
def get_list_of_clades(ptree: Phylo.BaseTree.TreeMixin):
    return list(
        map(
            lambda clade: list(
                (map(lambda x: x.name, clade.find_elements(terminal=True)))
            ),
            ptree.find_clades(),
        )
    )  # a list of list of terminals that define each clade.
