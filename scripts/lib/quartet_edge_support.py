...
def calculate_edge_support(
    tree: Phylo.BaseTree.TreeMixin, quartets: list[tuple[str, str, str, str]]
) -> dict[tuple[str, str], list[Tuple]]:
    lca = get_lcas(tree)
    dfs_order = get_dfs_order(tree)
    validate_tree(tree)
    supported_edges = {}
    dep = {n.id: n.dep for n in tree.find_elements(target=Phylo.BaseTree.Clade)}
    parent = {n.id: n.parent for n in tree.find_elements(target=Phylo.BaseTree.Clade)}
    name_to_id = {n.name: n.id for n in tree.find_clades(terminal=True)}
