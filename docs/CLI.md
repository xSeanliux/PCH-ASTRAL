...
- **`inference_registry.csv`** — one row per `(dataset, method)` run. Columns = the simulation join keys (`poly_level, character_count, min_tree_height, homoplasy_factor, horizontal_edges, model_tree, replica`) + `method, config_hash, method_config_json, runtime_seconds, point_estimate_newick, tree_set_path, consensus_method, fn_rate, fp_rate, status, ran_at, log_path`. Joins to `simulated_data_registry.csv` on the shared keys. Schema: `scripts/py/cli/schemata.py`.
