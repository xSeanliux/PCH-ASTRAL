...
| **ASTRAL** | `bash scripts/sh/runASTRAL3.sh -H R -i <csv> -o <dir> -q Q -b B -n N [-x]` | dataset CSV; requires MP4/GA outputs under `<dir>` for bipartitions (non-exact) | progress | `<dir>/PCH_ASTRAL_3(Q,B)/trees/N.tree` (Newick), `<dir>/PCH_ASTRAL_3(Q,B)/logs/N.log` | non-zero on ASTRAL failure |
